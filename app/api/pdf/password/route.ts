...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const operation = (formData.get('operation') as string) || 'encrypt';
    const password = (formData.get('password') as string) || '';
    tempDir = await getTempDir();
    const inputPath = await saveUploadedFile(file, tempDir);
    await ensureUserOutputDir(session.id);
    const outputFilename = `password_${uuidv4()}.pdf`;
    const outputPath = getUserOutputPath(session.id, outputFilename);
    const processor = new PDFProcessor();
    if (operation === 'encrypt') {
      await processor.encryptPdf(inputPath, outputPath, password);
    } else {
      await processor.decryptPdf(inputPath, outputPath, password);
    }
    ...
    return NextResponse.json({ success: false, error: err instanceof Error ? err.message : 'Şifreleme hatası' }, { status: 500 });
  }
}
