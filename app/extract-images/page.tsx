...
export default function ExtractImagesPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/pdf/extract-images', { method: 'POST', body: formData });
      if (!res.ok) {
      ...
      a.href = url;
      a.download = res.headers.get('content-disposition')?.includes('zip') ? 'extracted_images.zip' : 'extracted_image.png';
      a.click();
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <h1 className="text-2xl font-bold text-red-600 mb-2">Görsel Çıkarma</h1>
      <p className="text-slate-600 mb-6">PDF içindeki resimleri ayrı ayrı indirin.</p>
